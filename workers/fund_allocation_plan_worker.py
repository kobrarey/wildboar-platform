...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--live-read-only",
        action="store_true",
        help="Reserved for future stage. Not allowed in Stage 22.2.",
    )
...
def _validate_stage22_2_args(args: argparse.Namespace) -> None:
    if args.live_read_only:
        raise RuntimeError(
            "--live-read-only is reserved for a later stage. "
            "Stage 22.2 allows mock/fixture snapshots only."
        )
...
def _process_batch_in_own_session(
    *,
    ...
    try:
        ...
        snapshot = build_allocation_snapshot_from_fixture_file(
            fund_id=fund.id,
            fund_code=fund.code,
...
def _run_once(args: argparse.Namespace) -> int:
    ...
    log.info(
        ...
        "mock_snapshot_file=%s candidate_batches=%s",
        fund_code or "all",
        bool(args.dry_run),
...
def _run_loop(args: argparse.Namespace) -> int:
    ...
    log.info(
        "Allocation plan worker loop started sleep_sec=%s dry_run=%s mock_snapshot_file=%s",
        sleep_sec,
        bool(args.dry_run),
        args.mock_snapshot_file,
    )
...
def main() -> int:
    ...
    log.info(
        "Stage 22.2 allocation plan worker started. "
        "No real Bybit API calls, no trades, no transfers, no Earn stake."
    )
